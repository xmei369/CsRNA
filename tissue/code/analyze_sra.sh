...
if [[ ! -d $GENOME ]]; then
	...
	GENOME_GTF="$GENOME/Annotation/sczGene.gtf"
	GENOME_FA="$GENOME/Sequence/sczGenome.fa"
...
export -f is_paired
export -f dump_sra
find ~/ncbi/public/sra/*.sra | xargs --max-args=1 --max-procs=$N_CPUS -I {} bash -c 'dump_sra "$@"' _ {}
